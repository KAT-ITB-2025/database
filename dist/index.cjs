'use strict';var pgCore=require('drizzle-orm/pg-core'),cuid2=require('@paralleldrive/cuid2'),drizzleOrm=require('drizzle-orm');var k=cuid2.init({length:8}),l=()=>new Date;var $=pgCore.pgEnum("account_role_enum",["admin","mamet","mentor","user","guest"]),E=pgCore.pgTable("account",{id:pgCore.text("id").primaryKey(),nim:pgCore.text("nim").notNull().unique(),password:pgCore.text("password").notNull(),role:$("role").default("user").notNull(),createdAt:pgCore.timestamp("created_at").defaultNow(),updatedAt:pgCore.timestamp("updated_at").$onUpdate(l)}),le=drizzleOrm.relations(E,({one:e})=>({user:e(r,{fields:[E.id],references:[r.id]})}));var r=pgCore.pgTable("user",{id:pgCore.text("id").primaryKey().references(()=>E.id),nim:pgCore.text("nim").notNull().unique(),email:pgCore.text("email").unique(),fullName:pgCore.text("full_name"),birthDate:pgCore.date("birth_date"),phoneNumber:pgCore.text("phone_number"),idLine:pgCore.text("id_line"),idDiscord:pgCore.text("id_discord"),idInstagram:pgCore.text("id_instagram"),fakultas:pgCore.text("fakultas"),prodi:pgCore.text("prodi"),kelompok:pgCore.text("kelompok"),createdAt:pgCore.timestamp("created_at").defaultNow(),updatedAt:pgCore.timestamp("updated_at").$onUpdate(l)});var v=pgCore.pgEnum("media_bucket_enum",["core","oskm","dikpus","profile","submission-dikpus","submission-oskm","other"]),O=pgCore.pgTable("media",{id:pgCore.text("id").primaryKey().$defaultFn(k),creatorId:pgCore.text("creator_id").references(()=>r.id,{onDelete:"cascade"}).notNull(),name:pgCore.text("name").unique().notNull(),bucket:v("bucket").notNull().default("other"),type:pgCore.text("type").notNull(),url:pgCore.text("url").notNull(),updatedAt:pgCore.timestamp("updated_at",{withTimezone:true}).notNull().$onUpdate(l),createdAt:pgCore.timestamp("created_at",{withTimezone:true}).notNull().defaultNow()}),L=pgCore.pgTable("handbook",{mediaId:pgCore.text("media_id").primaryKey().references(()=>O.id),title:pgCore.text("title")}),Ne=drizzleOrm.relations(O,({one:e})=>({handbook:e(L,{fields:[O.id],references:[L.mediaId]})}));var V=pgCore.pgEnum("user_stage_progress_enum",["completed","in_progress","locked"]),i=pgCore.pgTable("stage",{id:pgCore.serial("id").primaryKey(),stageNumber:pgCore.integer("stage_number").notNull(),title:pgCore.text("title").notNull(),description:pgCore.text("description"),availableDate:pgCore.timestamp("available_date",{mode:"date",withTimezone:true}),deadlineDate:pgCore.timestamp("deadline_date",{mode:"date",withTimezone:true}),quizWeight:pgCore.real("quiz_weight").default(1).notNull(),roleplayWeight:pgCore.real("roleplay_weight").default(0).notNull(),lgdWeight:pgCore.real("lgd_weight").default(0).notNull(),stageWeight:pgCore.real("stage_weight").default(0).notNull(),createdAt:pgCore.timestamp("created_at").defaultNow(),updatedAt:pgCore.timestamp("updated_at").$onUpdate(l)}),q=pgCore.pgTable("user_stage_progress",{id:pgCore.text("id").primaryKey(),userId:pgCore.text("user_id").references(()=>r.id,{onDelete:"cascade"}),stageId:pgCore.serial("stage_id").references(()=>i.id,{onDelete:"cascade"}),status:V("status").default("in_progress").notNull(),completedAt:pgCore.timestamp("completed_at"),quizScore:pgCore.integer("quiz_score"),updatedAt:pgCore.timestamp("updated_at").$onUpdate(l)},e=>[pgCore.unique("user_stage_unique").on(e.userId,e.stageId)]),Oe=pgCore.pgTable("user_extended_score",{id:pgCore.text("id").primaryKey().references(()=>q.id),userId:pgCore.text("user_id").references(()=>r.id,{onDelete:"cascade"}),stageId:pgCore.serial("stage_id").references(()=>i.id,{onDelete:"cascade"}),stageNumber:pgCore.integer("stage_number").notNull(),roleplayScore:pgCore.integer("roleplay_score"),lgdScore:pgCore.integer("lgd_score")}),m=pgCore.pgTable("material",{id:pgCore.serial("id").primaryKey(),stageId:pgCore.serial("stage_id").references(()=>i.id,{onDelete:"cascade"}),profile:pgCore.text("profile")}),C=pgCore.pgTable("dialog",{id:pgCore.serial("id").primaryKey(),materialId:pgCore.serial("material_id").references(()=>m.id,{onDelete:"cascade"}),content:pgCore.text("content"),order:pgCore.integer("order").notNull()}),f=pgCore.pgTable("quiz",{id:pgCore.serial("id").primaryKey(),stageId:pgCore.serial("stage_id").references(()=>i.id,{onDelete:"cascade"})}),c=pgCore.pgTable("question",{id:pgCore.serial("id").primaryKey(),quizId:pgCore.serial("quiz_id").references(()=>f.id,{onDelete:"cascade"}),question:pgCore.text("question").notNull(),order:pgCore.integer("order").notNull()}),z=pgCore.pgTable("answer_option",{id:pgCore.serial("id").primaryKey(),questionId:pgCore.serial("question_id").references(()=>c.id),option:pgCore.text("option").notNull(),optionStatus:pgCore.boolean("option_status").notNull()}),Ce=drizzleOrm.relations(i,({many:e})=>({userStageProgresses:e(q),materials:e(m),quizzes:e(f)})),ze=drizzleOrm.relations(q,({one:e})=>({stage:e(i,{fields:[q.stageId],references:[i.id]})})),xe=drizzleOrm.relations(f,({one:e,many:S})=>({questions:S(c),stage:e(i,{fields:[f.stageId],references:[i.id]})})),ke=drizzleOrm.relations(c,({one:e})=>({quiz:e(f,{fields:[c.quizId],references:[f.id]})})),we=drizzleOrm.relations(c,({many:e})=>({answerOptions:e(z)})),he=drizzleOrm.relations(z,({one:e})=>({question:e(c,{fields:[z.questionId],references:[c.id]})})),Ie=drizzleOrm.relations(m,({one:e,many:S})=>({dialogs:S(C),stage:e(i,{fields:[m.stageId],references:[i.id]})})),Le=drizzleOrm.relations(C,({one:e})=>({material:e(m,{fields:[C.materialId],references:[m.id]})}));var X=pgCore.pgEnum("kelas_type_enum",["skill","issue"]),T=pgCore.pgTable("kelas",{id_kelas:pgCore.text("id_kelas").primaryKey(),kuota:pgCore.integer("kuota").notNull(),judul:pgCore.text("judul").notNull(),deskripsi:pgCore.text("deskripsi").notNull(),pembicara:pgCore.text("pembicara").notNull(),type:X("type").notNull()}),Ke=pgCore.pgTable("pemilihan_kelas",{user_id:pgCore.text("user_id").notNull().references(()=>r.id,{onDelete:"cascade"}),kelas_id:pgCore.text("kelas_id").notNull().references(()=>T.id_kelas,{onDelete:"cascade"}),waktu:pgCore.timestamp("waktu").defaultNow().notNull(),prioritas:pgCore.integer("prioritas").notNull()}),Re=pgCore.pgTable("peserta_kelas",{user_id:pgCore.text("user_id").notNull().references(()=>r.id,{onDelete:"cascade"}),kelas_id:pgCore.text("kelas_id").notNull().references(()=>T.id_kelas,{onDelete:"cascade"})});var Pe=pgCore.pgMaterializedView("user_stage_recaps",{userId:pgCore.text("user_id").primaryKey(),nim:pgCore.text("nim"),fullName:pgCore.text("full_name"),kelompok:pgCore.text("kelompok"),fakultas:pgCore.text("fakultas"),prodi:pgCore.text("prodi"),quiz1:pgCore.integer("quiz_1"),roleplay1:pgCore.integer("roleplay_1"),lgd1:pgCore.integer("lgd_1"),quiz2:pgCore.integer("quiz_2"),roleplay2:pgCore.integer("roleplay_2"),lgd2:pgCore.integer("lgd_2"),quiz3:pgCore.integer("quiz_3"),roleplay3:pgCore.integer("roleplay_3"),lgd3:pgCore.integer("lgd_3"),quiz4:pgCore.integer("quiz_4"),roleplay4:pgCore.integer("roleplay_4"),lgd4:pgCore.integer("lgd_4"),quiz5:pgCore.integer("quiz_5"),roleplay5:pgCore.integer("roleplay_5"),lgd5:pgCore.integer("lgd_5"),totalQuizScore:pgCore.integer("total_quiz_score"),totalRoleplayScore:pgCore.integer("total_roleplay_score"),totalLgdScore:pgCore.integer("total_lgd_score"),totalScore:pgCore.real("total_score")}).as(drizzleOrm.sql`
  SELECT
    u.id AS user_id,
    u.nim,
    u.full_name,
...
`);exports.account=E;exports.accountRelation=le;exports.accountRoleEnum=$;exports.answerOption=z;exports.answerOptionRelation=he;exports.dialog=C;exports.dialogRelation=Le;exports.handbook=L;exports.kelas=T;exports.kelasTypeEnum=X;exports.material=m;exports.materialRelation=Ie;exports.media=O;exports.mediaBucketEnum=v;exports.mediaRelation=Ne;exports.pemilihan_kelas=Ke;exports.peserta_kelas=Re;exports.question=c;exports.questionRelation=we;exports.questionsRelation=ke;exports.quiz=f;exports.quizRelation=xe;exports.stage=i;exports.stageRelation=Ce;exports.user=r;exports.userExtendedScore=Oe;exports.userStageProgress=q;exports.userStageProgressRelation=ze;exports.userStageProgressStatusEnum=V;exports.userStageRecaps=Pe;